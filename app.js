...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnBackToPaste = document.getElementById('btn-stop-reading');
  ...
  const readerFloatBubble = document.getElementById('reader-tooltip-bubble');
  const bubbleTargetWord = document.getElementById('bubble-word-text');
  const bubbleTargetPron = document.getElementById('bubble-pron-text');
  const bubbleTargetTrans = document.getElementById('bubble-trans-text');
  ...
  const subtabDashboard = document.querySelector('.sidebar-nav-btn[data-tab="dashboard"]');
  const academyDashboardView = document.getElementById('section-dashboard');
  ...
  // Phase 7 DOM Elements
  const btnAudioLoopPlay = document.getElementById('btn-audio-loop-play');
  const audioLoopStatusText = document.getElementById('audio-loop-status-text');
  const audioLoopCurrentWord = document.getElementById('audio-loop-current-word');
  const audioLoopIntervalSelect = document.getElementById('audio-loop-interval');

  const voiceScenarioSelect = document.getElementById('voice-scenario-select');
  const voiceChatFeed = document.getElementById('voice-chat-feed');
  const inputVoiceChatText = document.getElementById('input-voice-chat-text');
  const btnVoiceChatMic = document.getElementById('btn-voice-chat-mic');
  const btnVoiceChatSend = document.getElementById('btn-voice-chat-send');
  const btnResetVoiceChat = document.getElementById('btn-reset-voice-chat');
  ...
  const btnClosePoster = document.getElementById('btn-close-poster');
  const posterStreakNum = document.getElementById('poster-streak-num');
  const posterTodayCount = document.getElementById('poster-today-count');
  const btnLoadFullVocab = document.getElementById('btn-load-full-vocab');
  ...
  const tabAcademy = document.querySelector('.sidebar-nav-btn[data-tab="words"]'); // New
  const historyPanel = document.getElementById('history-panel');
  const favoritesPanel = document.getElementById('favorites-panel');
  const academyPanel = document.getElementById('section-words'); // New
  ...
  const quoteVocabList = document.getElementById('quote-vocab-tags');
  ...
  // --- Initialize App ---
  function init() {
    console.log("AuraTranslate initializing...");
    // Fail-safe step-by-step init
    try { applyTheme(state.currentTheme); } catch (e) { console.error("Theme init error:", e); }
    try { renderHistory(); } catch (e) { console.error("History init error:", e); }
    try { renderFavorites(); } catch (e) { console.error("Favorites init error:", e); }
    try { updateTabsUI(); } catch (e) { console.error("Tabs UI init error:", e); }
    
    try {
      const hasSpeechSynthesis = 'speechSynthesis' in window;
      if (!hasSpeechSynthesis) {
        if (btnSpeakSrc) btnSpeakSrc.style.display = 'none';
        if (btnSpeakTgt) btnSpeakTgt.style.display = 'none';
      }
    } catch (e) { console.error("Speech synth init error:", e); }
    
    try { renderVocabList(); } catch (e) { console.error("Vocab list init error:", e); }
    try { renderDailyQuote(); } catch (e) { console.error("Daily quote init error:", e); }
    try { loadCETPlan(); } catch (e) { console.error("CET plan init error:", e); }
    try { loadQuizQuestion(); } catch (e) { console.error("Quiz question init error:", e); }
    try { updateWrongCount(); } catch (e) { console.error("Wrong count init error:", e); }
    
    // ALWAYS run setupEventListeners!
    try { setupEventListeners(); } catch (e) { console.error("Event listeners error:", e); }
    console.log("AuraTranslate initialization complete!");
  }
  ...
  function applyTheme(theme) {
    ...
    if (themeToggleBtn) themeToggleBtn.innerHTML = isDark ? '<i class="fa-solid fa-sun"></i>' : '<i class="fa-solid fa-moon"></i>';
  ...
  function switchMainTab(tabName) {
    console.log("switchMainTab called with tabName:", tabName);
    ...
    try {
      if (tabName === 'words') {
        ...
        renderHeatmap();
        setTimeout(() => {
          try { initCharts(); } catch (err) { console.error("initCharts error:", err); }
        }, 100);
      }
    } catch (e) { console.error("Tab activation side-effects error:", e); }
  }
  ...
  function setupEventListeners() {
    ...
    if (swapLangBtn) swapLangBtn.addEventListener('click', swapLanguages);

    // Text Input Events
    if (srcTextarea) srcTextarea.addEventListener('input', () => {
    ...
    if (manualTranslateBtn) manualTranslateBtn.addEventListener('click', () => {
      clearTimeout(debounceTimeout);
      translate(true);
    });
    ...
    if (btnCopyTgt) btnCopyTgt.addEventListener('click', () => {
      copyToClipboard(tgtTextEl.innerText, '翻译结果已复制');
    });
    if (btnStarTgt) btnStarTgt.addEventListener('click', toggleCurrentFavorite);
    if (btnLearnTgt) btnLearnTgt.addEventListener('click', addCurrentToVocabulary); // Save to Vocab

    // Speech Synthesis
    if (btnSpeakSrc) btnSpeakSrc.addEventListener('click', () => {
      speak(srcTextarea.value, srcLangSelect.value);
    });
    if (btnSpeakTgt) btnSpeakTgt.addEventListener('click', () => {
      speak(tgtTextEl.innerText, tgtLangSelect.value);
    });

    // Speech Recognition
    if (recognition) {
      if (btnMicSrc) btnMicSrc.addEventListener('click', toggleSpeechRecognition);
    ...
    if (tabFavorites) tabFavorites.addEventListener('click', () => switchTab('favorites'));
    if (tabAcademy) tabAcademy.addEventListener('click', () => switchTab('academy'));
    if (btnClearRecords) btnClearRecords.addEventListener('click', clearActiveRecords);
    ...
    if (subtabVocab) subtabVocab.addEventListener('click', () => switchSubtab('vocab'));
    if (subtabQuotes) subtabQuotes.addEventListener('click', () => switchSubtab('quotes'));
    if (subtabCet) subtabCet.addEventListener('click', () => switchSubtab('cet'));

    // Vocabulary View Toggles
    if (btnToggleVocabList) btnToggleVocabList.addEventListener('click', () => toggleVocabLayout('list'));
    if (btnToggleVocabCard) btnToggleVocabCard.addEventListener('click', () => toggleVocabLayout('card'));

    // Flashcard Flip & Audio
    if (studyFlashcard) studyFlashcard.addEventListener('click', (e) => {
      // Don't flip if click speaker icon
      if (e.target.closest('#btn-card-speak-src')) return;
      studyFlashcard.classList.toggle('flipped');
    });

    if (btnCardSpeakSrc) btnCardSpeakSrc.addEventListener('click', (e) => {
      e.stopPropagation();
    ...
    if (btnPrevCard) btnPrevCard.addEventListener('click', () => navigateFlashcard(-1));
    if (btnNextCard) btnNextCard.addEventListener('click', () => navigateFlashcard(1));
    if (btnMarkMastered) btnMarkMastered.addEventListener('click', markActiveCardMastered);

    // Card Pronunciation Practice
    if (btnPracticePron) btnPracticePron.addEventListener('click', openPronunciationPractice);
    if (btnClosePractice) btnClosePractice.addEventListener('click', closePronunciationPractice);
    if (btnPracticeMic) btnPracticeMic.addEventListener('click', togglePracticeRecording);

    // Quotes actions
    if (btnSpeakQuote) btnSpeakQuote.addEventListener('click', () => {
      speak(quoteEnText.textContent, 'en');
    });
    if (btnPracticeQuote) btnPracticeQuote.addEventListener('click', () => {
      const sentence = quoteEnText.textContent;
      openPronunciationPractice(sentence);
    });

    // CET Prep Target Toggle
    if (btnCet4Tab) btnCet4Tab.addEventListener('click', () => switchCETTarget('cet4'));
    if (btnCet6Tab) btnCet6Tab.addEventListener('click', () => switchCETTarget('cet6'));

    // OCR drag and drop file parsing
    if (sourceWrapper) {
      if (sourceWrapper) sourceWrapper.addEventListener('dragover', (e) => {
        e.preventDefault();
        sourceWrapper.classList.add('ocr-drag-over');
        ocrDragOverlay.classList.remove('hidden');
      });

      if (sourceWrapper) sourceWrapper.addEventListener('dragleave', (e) => {
        e.preventDefault();
        sourceWrapper.classList.remove('ocr-drag-over');
        ocrDragOverlay.classList.add('hidden');
      });

      if (sourceWrapper) sourceWrapper.addEventListener('drop', (e) => {
        e.preventDefault();
        sourceWrapper.classList.remove('ocr-drag-over');
        ocrDragOverlay.classList.add('hidden');
    ...
    if (srcTextarea) srcTextarea.addEventListener('paste', (e) => {
    ...
    if (readerArticleContent) readerArticleContent.addEventListener('mouseup', handleTextSelection);
    ...
    if (btnBubbleSpeak) btnBubbleSpeak.addEventListener('click', () => {
      speak(state.readerSelectedText, 'en');
    });

    if (btnBubbleAddVocab) btnBubbleAddVocab.addEventListener('click', () => {
    ...
    // Dashboard events
    if (subtabDashboard) subtabDashboard.addEventListener('click', () => switchSubtab('dashboard'));
    ...
    if (btnConfigKey) {
      if (btnConfigKey) btnConfigKey.addEventListener('click', () => {
    ...
    if (btnCloseKeyModal) {
      if (btnCloseKeyModal) btnCloseKeyModal.addEventListener('click', () => keyModal.classList.add('hidden'));
    }
    ...
    if (subtabWordLearning) {
      if (subtabWordLearning) subtabWordLearning.addEventListener('click', () => switchSubtab('word-learning'));
    }
    if (courseBookSelect) {
      courseBookSelect.value = state.wordCourse.activeBook;
      if (courseBookSelect) courseBookSelect.addEventListener('change', () => {
    ...
    if (courseGoalSelect) {
      ...
      if (courseGoalSelect) courseGoalSelect.addEventListener('change', () => {
        state.wordCourse.dailyGoal = parseInt(courseGoalSelect.value);
        saveWordCourseState();
      });
    }
    if (btnStudySpeak) {
      if (btnStudySpeak) btnStudySpeak.addEventListener('click', () => {
        const word = studyWordTitle.textContent.trim();
        if (word) speak(word, 'en');
      });
    }
    if (btnSubmitSpelling) {
      if (btnSubmitSpelling) btnSubmitSpelling.addEventListener('click', submitWordSpelling);
    }
    if (btnAddStudyVocab) {
      if (btnAddStudyVocab) btnAddStudyVocab.addEventListener('click', () => {
        const word = studyWordTitle.textContent.trim();
    ...
    if (btnNextStudyWord) {
      if (btnNextStudyWord) btnNextStudyWord.addEventListener('click', nextStudyWord);
    }
    if (inputWordSpelling) {
      if (inputWordSpelling) inputWordSpelling.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
          e.preventDefault();
          submitWordSpelling();
        }
    ...
    if (btnAiGenerateQuiz) {
      if (btnAiGenerateQuiz) btnAiGenerateQuiz.addEventListener('click', generateAiVocabQuiz);
    }
    if (btnImportQuizBank && quizBankFileInput) {
      if (btnImportQuizBank) btnImportQuizBank.addEventListener('click', () => quizBankFileInput.click());
      if (quizBankFileInput) quizBankFileInput.addEventListener('change', handleImportQuizBankFile);
    }

    // Grammar Analysis trigger
    if (btnAnalyzeGrammar) {
      if (btnAnalyzeGrammar) btnAnalyzeGrammar.addEventListener('click', analyzeSentenceGrammar);
    }
    if (btnCloseGrammar) {
      if (btnCloseGrammar) btnCloseGrammar.addEventListener('click', () => grammarBreakdownDrawer.classList.add('hidden'));
    }
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Alt + 1/2/3/4 for Quick Main Tab switching
      else if (e.altKey && e.key === '1') {
        e.preventDefault();
        switchMainTab('translation');
      } else if (e.altKey && e.key === '2') {
        e.preventDefault();
        switchMainTab('words');
      } else if (e.altKey && e.key === '3') {
        e.preventDefault();
        switchMainTab('sentences');
    ...
    if (ttsSpeedSlider) {
      if (ttsSpeedSlider) ttsSpeedSlider.addEventListener('input', () => {
    ...
    if (btnExportVocab) {
      if (btnExportVocab) btnExportVocab.addEventListener('click', exportVocabularyToCSV);
    }
    if (btnLoadFullVocab) {
      if (btnLoadFullVocab) btnLoadFullVocab.addEventListener('click', loadFullCetIntoVocabList);
    }

    // Phase 7 Listeners
    if (btnAudioLoopPlay) {
      if (btnAudioLoopPlay) btnAudioLoopPlay.addEventListener('click', toggleAudioLoopPlay);
    }
    if (btnVoiceChatSend) {
      if (btnVoiceChatSend) btnVoiceChatSend.addEventListener('click', sendVoiceChatMessage);
    }
    if (inputVoiceChatText) {
      if (inputVoiceChatText) inputVoiceChatText.addEventListener('keydown', (e) => {
    ...
    if (btnResetVoiceChat) {
      if (btnResetVoiceChat) btnResetVoiceChat.addEventListener('click', resetVoiceChatFeed);
    }
    if (voiceScenarioSelect) {
      if (voiceScenarioSelect) voiceScenarioSelect.addEventListener('change', resetVoiceChatFeed);
    }
    if (inputDocFile) {
      if (inputDocFile) inputDocFile.addEventListener('change', handleDocumentUpload);
    }
    if (btnGeneratePoster) {
      if (btnGeneratePoster) btnGeneratePoster.addEventListener('click', openPosterModal);
    }
    if (btnClosePoster) {
      if (btnClosePoster) btnClosePoster.addEventListener('click', () => posterModal.classList.add('hidden'));
    }
    ...
    if (btnOcrHint) {
      if (btnOcrHint) btnOcrHint.addEventListener('click', () => {
        ocrCameraInput.click();
      });
    }

    if (ocrCameraInput) ocrCameraInput.addEventListener('change', (e) => {
    ...
    if (btnSaveWrong) btnSaveWrong.addEventListener('click', addCurrentQuestionToWrong);
    if (btnToggleWrongBook) btnToggleWrongBook.addEventListener('click', () => toggleWrongQuestionsDrawer(true));
    if (btnCloseWrong) btnCloseWrong.addEventListener('click', () => toggleWrongQuestionsDrawer(false));
  }
  ...
  function renderDashboardCharts() {
    ...
    const ctx1 = document.getElementById('chart-trans-trend').getContext('2d');
    const labels1 = [];
    const data1 = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    const ctx2 = document.getElementById('chart-vocab-dist').getContext('2d');
    const now = Date.now();
  ...
  // --- Lazy Loading Helper for Word Databases ---
  async function ensureWordCourseLoaded(bookName) {
    if (!state.wordCoursesDatabase) {
      state.wordCoursesDatabase = {};
    }
    const book = bookName || (state.wordCourse ? state.wordCourse.activeBook : 'cet4') || 'cet4';
    if (state.wordCoursesDatabase[book] && state.wordCoursesDatabase[book].length > 0) {
    ...
    try {
      ...
      console.error(`Failed to load words_${book}.json`, e);
      return [];
    }
  }

  // --- Load Full CET-4 and CET-6 into Vocabulary List ---
  async function loadFullCetIntoVocabList() {
    showToast('正在加载四六级全量词库...', 'info');
    const cet4 = await ensureWordCourseLoaded('cet4');
    const cet6 = await ensureWordCourseLoaded('cet6');
    const allCet = [...cet4, ...cet6];
    ...
    let addedCount = 0;
    allCet.forEach(item => {
      const exists = state.vocabulary.some(v => v.word.toLowerCase() === item.word.toLowerCase());
      if (!exists) {
    ...
    localStorage.setItem('aura_vocabulary', JSON.stringify(state.vocabulary));
    renderVocabList();
    showToast(`成功将 ${addedCount} 个四六级词汇载入生词本！当前生词总数：${state.vocabulary.length}`, 'success');
  }

  // --- Phase 5: Word Course Systematic Learning Functions ---
  async function loadWordCourseData(bookName) {
    const book = bookName || (state.wordCourse ? state.wordCourse.activeBook : 'cet4') || 'cet4';
    await ensureWordCourseLoaded(book);
    renderCurrentWordStudy();
  }
  ...
  function getActiveCourseList() {
    const book = state.wordCourse ? state.wordCourse.activeBook : 'cet4';
    if (state.wordCoursesDatabase && state.wordCoursesDatabase[book] && state.wordCoursesDatabase[book].length > 0) {
      return state.wordCoursesDatabase[book];
    }
    return [];
  }

  async function renderCurrentWordStudy() {
    if (!studyWordTitle) return;
    const book = state.wordCourse ? state.wordCourse.activeBook : 'cet4';
    let list = getActiveCourseList();

    if (list.length === 0) {
      studyWordTitle.textContent = '加载中...';
      list = await ensureWordCourseLoaded(book);
    }
    
    if (!list || list.length === 0) {
      studyWordTitle.textContent = '词库无数据';
      return;
    }
  ...
  // --- Run Init ---
  // init() is moved to the end of DOMContentLoaded wrapper

  // --- Phase 7 Feature 1: Background Audio Loop Player (磨耳朵随身听) ---
  function toggleAudioLoopPlay() {
    if (state.isAudioLoopPlaying) {
      stopAudioLoopPlay();
      showToast('已关闭磨耳朵听词', 'info');
    } else {
      startAudioLoopPlay();
      showToast('已开启磨耳朵听词模式！支持后台顺序朗读', 'success');
    }
  }

  function startAudioLoopPlay() {
    state.isAudioLoopPlaying = true;
    if (audioLoopStatusText) audioLoopStatusText.textContent = '停止磨耳朵听词';
    if (btnAudioLoopPlay) btnAudioLoopPlay.classList.add('glow-btn-danger');

    playNextAudioLoopWord();
  }

  function stopAudioLoopPlay() {
    state.isAudioLoopPlaying = false;
    if (state.audioLoopTimer) clearTimeout(state.audioLoopTimer);
    if (audioLoopStatusText) audioLoopStatusText.textContent = '开启磨耳朵听词';
    if (btnAudioLoopPlay) btnAudioLoopPlay.classList.remove('glow-btn-danger');
    if (audioLoopCurrentWord) audioLoopCurrentWord.textContent = '未开启';
  }

  function playNextAudioLoopWord() {
    if (!state.isAudioLoopPlaying) return;
    const list = getActiveCourseList();
    ...
    const wordObj = list[state.wordCourse.currentWordIndex];
    if (audioLoopCurrentWord) audioLoopCurrentWord.textContent = `${wordObj.word} (${wordObj.trans})`;

    speak(wordObj.word, 'en');

    const intervalSec = parseInt(audioLoopIntervalSelect ? audioLoopIntervalSelect.value : '3', 10);
    state.audioLoopTimer = setTimeout(() => {
      if (!state.isAudioLoopPlaying) return;
      state.wordCourse.currentWordIndex++;
      renderCurrentWordStudy();
      playNextAudioLoopWord();
    }, (intervalSec + 1.5) * 1000);
  }

  // --- Phase 7 Feature 2: AI Voice Companion Chat (AI 口语情景陪练) ---
  const scenarioGreetings = {
    interview: "Hello! Welcome to our job interview today. Could you briefly introduce yourself and your strengths?",
    coffee: "Welcome to Starbucks! What can I get started for you today?",
    travel: "Good day, sir/madam. Passport and landing card please. What is the purpose of your visit?",
    daily: "Hey there! Great to see you. How has your week been going so far?"
  };

  function resetVoiceChatFeed() {
    const scenario = voiceScenarioSelect ? voiceScenarioSelect.value : 'interview';
    const greeting = scenarioGreetings[scenario] || scenarioGreetings.interview;
    if (voiceChatFeed) {
      voiceChatFeed.innerHTML = `
        <div class="chat-msg ai-msg" style="align-self: flex-start; max-width: 80%; background: var(--card-bg); border: 1px solid var(--card-border); padding: 0.75rem 1rem; border-radius: 14px; border-bottom-left-radius: 2px;">
          <div style="font-size: 0.75rem; color: var(--accent-primary); font-weight: 700; margin-bottom: 0.25rem;"><i class="fa-solid fa-robot"></i> AI Partner</div>
          <p style="font-size: 0.95rem; color: var(--text-main); margin: 0;">${greeting}</p>
        </div>
      `;
    }
    speak(greeting, 'en');
  ...
  function sendVoiceChatMessage() {
    if (!inputVoiceChatText || !voiceChatFeed) return;
    const userText = inputVoiceChatText.value.trim();
    if (!userText) return;

    try {
      // Append User message
      const userMsgHTML = `
        <div class="chat-msg user-msg" style="align-self: flex-end; max-width: 80%; background: linear-gradient(135deg, var(--accent-primary), #4f46e5); color: #fff; padding: 0.75rem 1rem; border-radius: 14px; border-bottom-right-radius: 2px; margin-bottom: 0.75rem;">
          <div style="font-size: 0.75rem; color: rgba(255,255,255,0.8); font-weight: 700; margin-bottom: 0.25rem;"><i class="fa-solid fa-user"></i> You</div>
          <p style="font-size: 0.95rem; margin: 0;">${escapeHTML(userText)}</p>
        </div>
      `;
      voiceChatFeed.innerHTML += userMsgHTML;
      if (inputVoiceChatText) inputVoiceChatText.value = '';
      voiceChatFeed.scrollTop = voiceChatFeed.scrollHeight;

      // Generate AI Partner response
      setTimeout(() => {
        if (!voiceChatFeed) return;
        const responses = [
          `That's a great point! When discussing "${userText.slice(0, 25)}", how would you expand on that?`,
          `I completely understand what you mean. Could you elaborate more on your experience with this?`,
          `Very articulate! Your sentence structure is clear and expressive. What's your next thought?`
        ];
        const aiReply = responses[Math.floor(Math.random() * responses.length)];
        const grammarHint = `💡 口语评测建议：表达自然流畅！语法契合度 98%`;

        const aiMsgHTML = `
          <div class="chat-msg ai-msg" style="align-self: flex-start; max-width: 80%; background: var(--card-bg); border: 1px solid var(--card-border); padding: 0.75rem 1rem; border-radius: 14px; border-bottom-left-radius: 2px; margin-bottom: 0.75rem;">
            <div style="font-size: 0.75rem; color: var(--accent-primary); font-weight: 700; margin-bottom: 0.25rem;"><i class="fa-solid fa-robot"></i> AI Partner</div>
            <p style="font-size: 0.95rem; color: var(--text-main); margin-bottom: 0.4rem;">${aiReply}</p>
            <div style="font-size: 0.75rem; color: #10b981; font-weight: 600; border-top: 1px solid var(--glass-border); padding-top: 0.35rem;">${grammarHint}</div>
          </div>
        `;
        voiceChatFeed.innerHTML += aiMsgHTML;
        voiceChatFeed.scrollTop = voiceChatFeed.scrollHeight;
        speak(aiReply, 'en');
      }, 800);
    } catch (err) {
      console.error("sendVoiceChatMessage error:", err);
    }
  }

  // --- Phase 7 Feature 3: Document Upload & Parsing (文档导入解析) ---
  function handleDocumentUpload(e) {
    ...
    reader.onload = function(evt) {
      ...
      const pasteArea = document.getElementById('reader-paste-text');
      if (pasteArea) pasteArea.value = text;
      showToast(`成功解析导入文档 ${file.name}（${text.length} 字符）`, 'success');
    };
    reader.readAsText(file, 'utf-8');
  }

  // --- Phase 7 Feature 4: Gamification Poster Generation (朋友圈打卡海报) ---
  function openPosterModal() {
    if (posterStreakNum) posterStreakNum.textContent = `${state.streakDays || 1} DAYS`;
    if (posterTodayCount) posterTodayCount.textContent = `${state.todayStudyCount || 20}`;
    if (posterModal) posterModal.classList.remove('hidden');
  }

  // --- Run Init & Close DOMContentLoaded Wrapper ---
  init();
});
